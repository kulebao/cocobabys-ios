...
- (void)initProgressBar
{
    ...
    [SBCaptureToolKit setView:_progressBar toOriginY:CGRectGetMaxY(_recorder.preViewLayer.frame)];
...
- (void)initTopLayout
{
    ...
    self.closeButton.frame = CGRectMake(10, 5, 135, buttonW);
    ...
    [SBCaptureToolKit setView:self.sendButton
                     toOrigin:CGPointMake(self.view.frame.size.width-self.sendButton.frame.size.width-10, 5)];
    self.sendButton.hidden = YES;
    [self.view insertSubview:_sendButton belowSubview:_maskView];
...
- (void)pressCloseButton
{
    if ([_recorder getVideoCount] > 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:@"放弃这段视频?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"放弃", nil];
...
- (UIView *)getMaskView
{
    ...
    maskView.backgroundColor = color(30, 30, 30, 0.5);
...
- (void)initPlayLayer
{
    ...
    _playerLayer.frame = _recorder.preViewLayer.frame;
