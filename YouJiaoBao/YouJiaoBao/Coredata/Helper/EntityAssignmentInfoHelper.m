...
@implementation EntityAssignmentInfoHelper

+ (EntityAssignmentInfo*)queryEntityWithAssignmentId:(NSNumber*)uid {
    EntityAssignmentInfo* entity = nil;
    if (uid) {
        NSManagedObjectContext* context = [[CSCoreDataHelper sharedInstance] managedObjectContext];
        
        NSFetchRequest* fr = [[NSFetchRequest alloc] initWithEntityName:@"EntityAssignmentInfo"];
...
+ (NSArray*)updateEntities:(id)jsonObjectList {
    ...
    for (NSDictionary* jsonObject in jsonObjectList) {
        ...
        if (uid.integerValue > 0) {
            EntityAssignmentInfo* entity = [self queryEntityWithAssignmentId:uid];
            BOOL updated = NO;
            if (entity == nil) {
                entity = [NSEntityDescription insertNewObjectForEntityForName:@"EntityAssignmentInfo" inManagedObjectContext:context];
                entity.uid = uid;
                updated = YES;
            }
            else {
            ...
            if (updated) {
                ...
                entity.iconUrl = [jsonObject objectForKey:@"icon_url"];
